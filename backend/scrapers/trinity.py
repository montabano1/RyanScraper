...
class TrinityScraper(BaseScraper):
    def __init__(self):
        super().__init__('trinity')
        self.start_url = "https://www.trinity-partners.com/listings"
    ...
    async def _get_iframe_url(self, crawler):
        """Get the iframe URL from Trinity Partners property page."""
        ...
        for row in soup.select('.js-lease-space-row-toggle.spaces'):
            ...
            if len(cells) >= 5:
                unit = {
                    ...
                    "address": address,
                    "location": location,
                    "listing_url": f"https://www.trinity-partners.com/listings/?propertyId={url.split('propertyId=')[1].split('&')[0]}&tab=spaces",
